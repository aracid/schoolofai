...
def visualize_layer(model,
    ...
    def _generate_filter_image(input_img,
        ...
        # Next K.function returns a tensor function and not a model object where you can use predict().
        # The correct way of using is just to call as a function
    ...
    def _draw_filters(filters, n=None):
        ...
        MARGIN = 0
        stitched_filters = np.zeros((512, 512, 3), dtype='uint8')
        if (len(filters)> 0):
            width = n * filters[0][0].shape[0]  + (n - 1) * MARGIN
            height = n * filters[0][0].shape[1]  + (n - 1) * MARGIN
    ...
    # Make sure that the output layer is a conv2D layer
...
void main()
{
    gl_FragColor = texture2D(colormaps, v_texcoord);
}
"""
...
if __name__ == '__main__':
    ...
    # model_path = 'ssd7_epoch-80_loss-2.1001_val_loss-1.6474.h5'
    # model = load_model(model_path, custom_objects={'AnchorBoxes': AnchorBoxes,
    #                                                'compute_loss': SSDLoss(neg_pos_ratio=3, alpha=1.0).compute_loss})


    model_path = 'ssd300_pascal_07+12_epoch-49_loss-3.1832_val_loss-2.7290.h5'
    ...
    model.summary()
    layersToView = []
    for eachlayer in model.layers[10:12]:
        if isinstance(eachlayer, layers.Conv2D):
            print ("Extracting gradient for layer : %s" % eachlayer.name)
            layersToView.append([visualize_layer(model, layer_name=eachlayer.name), eachlayer.name])
    # layersToView.append([visualize_layer(model, layer_name='conv7_2'), 'conv7_2'])
    # example function call
    # colormaps = visualize_layer(model, layer_name='conv3')
    print ("layersToView : " , len(layersToView))
    canvas = Canvas(colormaps=layersToView)
    app.run()